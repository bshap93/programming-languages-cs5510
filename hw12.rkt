...
#|
  ...
  22  [doBoxK (k : Cont)]
  23  [doUnboxK (k : Cont)]
  25  [setBoxArgK (b : ExprD)
                  (env : Env)
                  (k : Cont)]
  26  [doSetBoxK (val : Value)
                 (k : Cont)])
|#
...
(module+ test
  ...
  (test (parse '{box 1}) (boxC (numC 1)))
  (test (parse '{unbox 1}) (unboxC (numC 1)))
  (test (parse '{set-box! 1 1}) (setboxC (numC 1) (numC 1)))
...
(define (continue)
  (case (ref k-reg 0)
    ...
    [(26) ; doSetBoxK
     (begin
       (vector-set! memory (+ v-reg 1) (ref k-reg 1)) ; change the pointer
       (set! k-reg (ref k-reg 2))
       (continue))]))
...
(module+ test
  ...
  (reset!)
  (vtest (interpx (compile
                   (parse '{unbox {set-box! {box 8} {+ {unbox {box 10}} 12}}})
                   mt-env)
                  empty-env
                  (init-k))
         (numV 22))
