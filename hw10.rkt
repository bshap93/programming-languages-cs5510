...
(define (interp [a : ExprC] [env : Env]) : Value
  (type-case ExprC a
    ...
    [if0C (tst thn els) (if (num-zero? (interp tst env))
                            (interp thn env)
                            (interp els env))]
    [lamC (n t body)
          (closV n body env)]
...
(module+ test
  ...
  (test (interp (parse '{if0 1 1 2})
                mt-env)
        (numV 2))
  (test/exn (interp (parse '{if0 empty 1 2})
                    mt-env)
            "not a number")
...
(define (typecheck [a : ExprC] [tenv : TypeEnv])
  (type-case ExprC a
    ...
    [if0C (tst thn els)
          (let ([t1 (typecheck thn tenv)])
            (begin
              (unify! (typecheck tst tenv) (numT) tst)
              (unify! t1 (typecheck els tenv) a)
              t1))]
    ...
    [consC (l r)
           (let ([t1 (typecheck l tenv)])
             (let ([t2 (typecheck r tenv)])
               (begin
                 (unify! (listofT t1) t2 a)
                 t2)))]
    [firstC (lis)
            (let ([t1 (typecheck lis tenv)])
              (let ([return-type (varT (box (none)))])
                (begin
                  (unify! (listofT return-type) t1 a)
                  return-type)))]
    [restC (lis)
           (let ([t1 (typecheck lis tenv)])
             (let ([unknown-type (varT (box (none)))])
               (begin
                 (unify! (listofT unknown-type) t1 a)
                 t1)))]))
...
(module+ test
  ...
  (test (typecheck (parse '{if0 1 2 3}) mt-env)
        (numT))
  (test (typecheck (parse '{if0 1
                                {lambda {[x : num]} {+ x 3}}
                                {lambda {[x : num]} {+ x 4}}})
                   mt-env)
        (arrowT (numT) (numT)))
  ...
  (test/exn (typecheck (parse '{if0 {lambda {[x : num]} 3} 1 2}) mt-env)
            "no type")
  (test/exn (typecheck (parse '{if0 0 {lambda {[x : num]} 3} 2}) mt-env)
            "no type")
...
;; run-prog ------------------------------------------------
(define (run-prog [a : s-expression]) : s-expression
  (let ([expr (parse a)])
    (begin
      (typecheck expr mt-env)
      (type-case Value (interp expr mt-env)
