...
;; parse ----------------------------------------
(define (parse [s : s-expression] [pat : s-expression] [s-exp-> : (s-expression -> 'a)]) : (ExprC 'a)
  (cond
...
;; interp ----------------------------------------
(define (interp [a : (ExprC 'a)] [env : Env] [lit+ : ('a 'a -> 'a)] [lit* : ('a 'a -> 'a)]) : (Value 'a)
  (type-case (ExprC 'a) a
    [litC (n) (litV n)]
    [idC (s) (lookup s env)]
    ...
    [lamC (n body)
          (closV n body env)]
    [appC (fun arg) (type-case (Value 'a) (interp fun env lit+ lit*)
                      [closV (n body c-env)
                             (interp body
                                     (extend-env
                                      (bind n
